...
def setup_platform(hass, config, add_devices,
    ...
    add_devices([FaceSensor(name, camera_entity_id, port, token, baidu_client, group_list, options, DEFAULT_REQUEST_TYPE)])

class FaceSensor(Entity):

    def __init__(self, name, camera_entity_id, port, token, baidu_client:AipFace, group_list, options, request_type):
